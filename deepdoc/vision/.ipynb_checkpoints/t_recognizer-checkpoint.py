...
def get_sorted_indices(arr):
    # 创建 (值 -> 索引) 的映射，注意如果有重复值会被覆盖，除非用排序稳定的方法
    indices = {v: i for i, v in enumerate(sorted(arr))}
    return [indices[v] for v in arr]
...
def main(args):
    ...
    for i, lyt in enumerate(layouts):
        ...
        idx = [t["bbox"][1] for t in lyt]
        idx = get_sorted_indices(idx)
        for jj in range(len(lyt)):
            cropped_image = images[0].crop(lyt[jj]["bbox"])
            cropped_image.save(f"/root/outs2/cropped_result_{idx[jj]}.png")

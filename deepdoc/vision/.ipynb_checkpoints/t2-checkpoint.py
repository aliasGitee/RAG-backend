...
def find_nearest_preceding(current_index, sorted_blocks, label):
    """Finds the nearest block with a specific label that precedes the current block."""
    for i in range(current_index - 1, -1, -1):
        if sorted_blocks[i].get("label") == label:
            return sorted_blocks[i]
    return None

def find_nearest_subsequent(current_index, sorted_blocks, label):
    """Finds the nearest block with a specific label that follows the current block."""
    for i in range(current_index + 1, len(sorted_blocks)):
        if sorted_blocks[i].get("label") == label:
            return sorted_blocks[i]
    return None
...
def main(args):
    ...
    if args.mode.lower() == "layout":
        ...
        for i, lyt in enumerate(layouts):
            # Sort blocks by their vertical position to make finding nearest blocks easier
            lyt_sorted = sorted(lyt, key=lambda b: b['bbox'][1])
            
            for j, block in enumerate(lyt_sorted):
                ...
                if block_label == 'figure':
                    figure_count += 1
                    output_filename_base = f"figure_{figure_count}"
                    is_figure = True
                
                elif block_label == 'table':
                    table_count += 1
                    output_filename_base = f"table_{table_count}"
                    is_table = True

                if output_filename_base:
                    # 1. Crop and encode the main block image
                    block_img = crop_image(images[i], block['bbox'])
                    json_data['pic'] = image_to_base64(block_img)

                    # 2. Find the nearest preceding title and get its text via OCR
                    title_block = find_nearest_preceding(j, lyt_sorted, 'title')
                    if title_block:
                        title_img = crop_image(images[i], title_block['bbox'])
                        title_img_np = np.array(title_img)
                        ocr_results = ocr(title_img_np)
                        title_text = " ".join([res[1][0] for res in ocr_results if res])
                        if title_text:
                            json_data['title'] = title_text

                    # 3. Find the corresponding caption block and encode its image
                    if is_figure:
                        caption_block = find_nearest_subsequent(j, lyt_sorted, 'figure caption')
                        if caption_block:
                            caption_img = crop_image(images[i], caption_block['bbox'])
                            json_data['figure caption'] = image_to_base64(caption_img)
                    
                    elif is_table:
                        caption_block = find_nearest_preceding(j, lyt_sorted, 'table caption')
                        if caption_block:
                            caption_img = crop_image(images[i], caption_block['bbox'])
                            json_data['table caption'] = image_to_base64(caption_img)

                    # 4. Save the collected data as a JSON file
                    json_filepath = os.path.join(args.output_dir, f"{output_filename_base}.json")
                    with open(json_filepath, 'w', encoding='utf-8') as f:
                        json.dump(json_data, f, indent=4, ensure_ascii=False)
                    logging.info(f"Saved layout block info to: {json_filepath}")

            # Keep the original functionality of saving the annotated image for the page
        ...
        for i, lyt in enumerate(layouts):
            ...
            img = draw_box(images[i], lyt_processed, detr.labels, float(args.threshold))
            img.save(outputs[i], quality=95)
            logging.info("save result to: " + outputs[i])
